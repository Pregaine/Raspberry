    ...
    def GetDF( self ):
        ...
        try:            
            ...
            if u'您要尋找的資源有問題而無法顯示' in response.text:
                print( '股號 {} 您要尋找的資源有問題而無法顯示'.format( self.number ) )
                return False
...
def CompareFileModifyTime( path, hour = 12 ):
...
def main( ):
    ...
    for stock in empty_lst:
        if CompareFileModifyTime( './技術指標/week/{0}_周線技術指標.csv'.format( stock ) ):
            continue
