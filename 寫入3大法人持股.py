...
class DB_Investors :
    ...
    def FindDuplicate( self, data ):
        ...
        # 尋找重覆資料
        cmd = '''SELECT stock, date from mysql.INVESTORS where stock = \'{}\' and date = \'{}\';'''.format( self.stock, data )
        ...
        if ft is not None:
            ...
            cmd = '''DELETE FROM mysql.INVESTORS where stock = \'{}\' and date = \'{}\';'''.format( self.stock, data )
    ...
    def WriteDB( self ):
        ...
        self.df.columns = [ 'stock', 'date', 'foreign_sell', 'investment_sell',
                            'dealer_sell', 'single_day_sell', 'foreign_estimate',
                            'investment_estimate',
                            'dealer_estimate',
                            'single_day_estimate',
                            'foreign_ratio',
        ...
        self.df.to_sql( name = 'INVESTORS', 
                        con = enginePi, 
                        index = False, 
                        if_exists = 'append', 
                        index_label = None )
                        
        print( '寫入資料庫{0:>2}'.format( self.stock ) )
